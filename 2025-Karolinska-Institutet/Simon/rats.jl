...
# ╔═╡ db74fe90-269b-11f0-1ef0-f322d675f9f6
using Turing, RDatasets, DataFrames, LinearAlgebra, Chain

# ╔═╡ 72008119-502d-48f0-a0f5-be4a5f5ae7fa
using LogExpFunctions: logistic

# ╔═╡ c8592e58-38ee-4722-b585-a21644893484
df_rats = dataset("survival", "rats")

# ╔═╡ b586e7e8-685b-483a-9945-581f869b034f
@model function logistic_regression(X, y)
	# Design matrix of size n × m
	n, m = size(X)

	# Priors
	β ~ MvNormal(zeros(m), 10 * I)

	# Likelihood
	p = [Xᵢ' * β for Xᵢ in eachrow(X)]
	for i in eachindex(y)
		y[i] ~ BernoulliLogit(p[i])
	end

	return (; p)
end

# ╔═╡ 10794ae3-5f6a-4a11-a0bf-2c046e0701a5
X = @chain df_rats begin
	getproperty(_, :Sex)
	_ .== "f" # female is 1
	hcat(ones(length(_)), _)
end

# ╔═╡ b07d8a14-df4f-4186-b90e-094483d5563b
y = df_rats.Status

# ╔═╡ 4e91ae85-aac8-45e6-95ae-697f34494657
model = logistic_regression(X, y)

# ╔═╡ 66afe99f-2fa3-4131-969a-5d37ced1b08d
chn = sample(model, NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ c9d45b4c-ae1c-45af-a15d-d865377ae72a
chn

# ╔═╡ aa6ddad0-8923-45ea-aadf-ec2f27a6ef10
describe(chn)
...
manifest_format = "2.0"
project_hash = "2bfbee223e5cb8312e413505bb084d086c2e5b6d"
