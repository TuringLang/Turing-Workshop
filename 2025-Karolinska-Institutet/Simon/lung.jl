...
# ╔═╡ 1a858ea2-2699-11f0-3608-19da7b89c27a
using Turing, RDatasets, DataFrames, LinearAlgebra, Chain

# ╔═╡ d9c568cb-8d73-4939-b36a-bcd77b482a64
df_lung = dataset("survival", "lung");

# ╔═╡ dafe9cf5-f8d6-4631-8892-d7c86035444b
@model function censored_survival_model(cens, X, y)
	# Size of the predictor matrix
	n, p = size(X)
	...
	# Likelihood
	θ = [exp(Xᵢ' * β) for Xᵢ in eachrow(X)]
	
	for i in eachindex(y)
		if iszero(α) # Don't know why I need this since pdf(LogNormal(0, 2), 0) == 0?
			Turing.@addlogprob! -Inf
			continue
		end
		if cens[i]
			Turing.@addlogprob! logccdf(Weibull(α, θ[i]), y[i])
		else
			y[i] ~ Weibull(α, θ[i])
		end
	end
...
# ╔═╡ 5f9ba9e2-6b80-48bc-985b-4f94f7bda41b
df_model = @chain df_lung begin
	dropmissing(_, [:Time, :Status, :Age, :Sex])
	DataFrames.transform(_, :Sex => ByRow(==(2)) => identity) # sex = 1 is female
end

# ╔═╡ b0071be1-f5be-48cb-a2b7-7dcee826aa90
standardise(x) = (x .- mean(x)) ./ std(x)

# ╔═╡ 967752ea-fc97-4484-9ff3-ce32ffd39961
X = @chain df_model begin
	select(_, :Age, :Sex)
	transform(_, :Age => standardise => identity)
	Matrix(_)
	hcat(ones(size(_, 1)), _) # Column of ones for intercept in design matrix
end

# ╔═╡ f063652c-6dc9-4ea7-a4d6-b70811acd025
cens = df_model.Status .== 1

# ╔═╡ 84cd7ad6-f5c1-4d58-838d-0ecbba1fcb85
y = df_model.Time

# ╔═╡ 5508e2ac-a9d1-48f8-8307-06fecbdbe5b0
model = censored_survival_model(cens, X, y)

# ╔═╡ 4057526b-8b89-4c7f-aa7c-6dbc08fa261f
chn = sample(model, NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 0711147d-28d4-4ad9-8167-740cf35a77eb
describe(chn)
...
manifest_format = "2.0"
project_hash = "ca6c894de8914743558ebbd16c711f5072af6a4f"
