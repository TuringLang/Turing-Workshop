...
- [ ] Add some more stuff to TuringCallbacks.jl sectione
- [X] For condtioning:
  - [X] Maybe =missing=
...
Turing.jl performs no analysis of the code, and thus cannot know that =y= is a constant
...
But you will encounter the other two approaches in the wild
...
A =fixed= variable is not included in the log joint
...
=NegativeBinomial2(μ, ϕ)= is parameterized by the mean $μ$ and /dispersion/ $\phi$
